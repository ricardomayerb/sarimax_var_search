...
models_rmse_at_each_h_arima_stata_sel  <- as_tibble(
  each_h_just_model_and_ave_rmse_sarimax) %>% 
...
models_rmse_at_each_h_stata_sel <- rbind(models_rmse_at_each_h_var, 
                               models_rmse_at_each_h_arima_stata_sel) %>% 
  mutate(inv_mse = 1/rmse^2) %>% 
  group_by(rmse_h) %>% 
  mutate(rank_h = rank(rmse)) %>% 
  arrange(rmse_h, rank_h)
...
ffall_ssel <- indiv_weigthed_fcs(tbl_of_models_and_rmse = models_rmse_at_each_h_stata_sel,
                            h = h_max, extended_x_data_ts = extended_x_data_ts,
                            rgdp_ts_in_arima = rgdp_ts_in_arima,
                            max_rank_h = 30)
...
ffall_arima_ssel <- indiv_weigthed_fcs(tbl_of_models_and_rmse = models_rmse_at_each_h_stata_sel,
                                  h = h_max, extended_x_data_ts = extended_x_data_ts,
                                  rgdp_ts_in_arima = rgdp_ts_in_arima,
                                  model_type = "Arima")
