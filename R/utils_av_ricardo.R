...
fit_arimas <- function(y_ts, auto = FALSE, order_list = NULL, 
  ...
  for (i in seq.int(1, n_of_series)) {
    ...
    if (!auto) {
      ...
      print(paste("fitting auto arima for variable:", this_arima_names[i]))
      print(paste("do_stepwise:", do_stepwise))
      print(paste("do_approximation:", do_approximation))
      
      fit <- try(auto.arima(y = this_y, lambda = my_lambda, 
                            biasadj = my_biasadj, 
      ...
      if (class_fit == "try-error") {
        ...
        new_method <-  "CSS-ML"
        print("in fit arima, ML failed")
        
        fit <- try(auto.arima(y = this_y, 
                              lambda = my_lambda, 
                              biasadj = my_biasadj,
                              stepwise = do_stepwise, 
                              approximation = do_approximation))
        ...
        if (class_fit == "try-error") {
          ...
          warning(this_mssg)
          print("in fit arima, CSS-ML failed in auto.arima")
...
get_extended_one_monthly_variable <- function(
  ...
  start_date = NULL,
  auto_do_step_wise = FALSE,
  auto_do_aproximation = FALSE) {
  ...
  if (do_auto) {
    ...
    fit_arima_monthly <- fit_arimas(
      ...
      method = method, 
      auto = do_auto,
      do_stepwise = auto_do_step_wise,
      do_approximation = auto_do_aproximation)
...
get_extended_monthly_variables <- function(
  ...
  print_comments_on_constant = FALSE,
  final_forecast_horizon = c(2019, 12), method = "ML",
  auto_do_step_wise = FALSE, auto_do_aproximation = FALSE) {
...
my_arimax <- function(y_ts, xreg_ts, y_order, y_seasonal,
                      ...
                      procrust_start = NULL, procrust_end = NULL) {
