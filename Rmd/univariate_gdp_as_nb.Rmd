...
o_dm <- as_tibble(cbind(model = "dm_drift", dm_orders))
o_as <- as_tibble(cbind(model = "auto_s", t(ord_auto_s)))
...
```{r fit_results, echo=FALSE}
aco_as <- cbind(model = "auto_s", sweep::sw_tidy(arima_log_fit_auto_slow))
aco_a <- cbind(model = "auto", sweep::sw_tidy(arima_log_fit_auto))
aco_d <- cbind(model = "dm_drift", sweep::sw_tidy(arima_log_fit_drift))
aco <- cbind(model = "dm_nodrift", sweep::sw_tidy(arima_log_fit))
...
```{r k_show_parameter_estimates, echo=FALSE}
...
```{r arima_based_forecasts, echo=FALSE}
