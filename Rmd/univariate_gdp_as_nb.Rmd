...
```{r tscv_arimas}
fautoarima <- function(x, h) {
  forecast(auto.arima(x, approximation = FALSE, stepwise = FALSE, 
                      include.drift = TRUE), h=h)
}

farima <- function(x, h) {
  forecast(auto.arima(x, stepwise = FALSE, allowdrift = TRUE, approximation = FALSE), h=h)
}
